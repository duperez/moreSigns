...
- New tags:
  - `moresigns:flat_on_cutting_board`: Items which will always render flat and 2D when placed on a Cutting Board, to fix certain render cases.
...
- New tags:
  - `moresigns:terrain`: Blocks commonly present in biome surfaces. Populated by `minecraft:dirt` and `minecraft:sand`, and used for wild crop generation;
  - `moresigns:horse_feed_tempted`: Entities which will be tempted by Horse Feed when held;
  - `moresigns:serving_containers`: Items commonly used as containers for Cooking Pot recipes. Used for shift-click logic in the UI;
...
- CraftTweaker integration has been re-enabled (thanks, Witixin1512!):
  - This integration exposes `recipe_book_tab` and `moresigns:tool_action` to CT scripts;
...
- New tags: 
  ...
  - `moresigns:cabinets`: This, together with `cabinets/wooden`, is used to classify cabinet items for searching, crafting and integration;
...
### Additions
- `moresigns:straw_blocks` - Tag for blocks made (predominantly) out of Straw. Mineable with knives by default;
...
- `moresigns:mineable/knife` has been properly configured:
...
### Breaking Changes
- Read this: https://github.com/vectorwing/moresigns/wiki/1.0-Breaking-Changes
...
- https://github.com/vectorwing/moresigns/wiki/0.5-Breaking-Changes
...
- Cabbage Rolls now consult the tag `moresigns:cabbage_roll_ingredients` for valid fillings (thank you, bagel!);
...
- Cutting Board recipes now make use of a ToolIngredient, freeing the `type` parameter and allowing custom ingredients (thank you, ochotonida!)
  ...
  - https://github.com/vectorwing/moresigns/wiki/Cutting-Board-Recipes
...
- Updated Organic Compost and Rich Soil:
  ...
  - Rich Soil now checks the tag `moresigns:unaffected_by_rich_soil` for blocks that shouldn't receive growth boosts;
...
- Straw harvesting now checks "moresigns:straw_harvesters" for items that can harvest straw from grassy plants.
